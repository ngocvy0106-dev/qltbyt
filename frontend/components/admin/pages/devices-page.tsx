...
function splitDepartmentAndLocation(rawLocation: string) {
  ...
  const isFirstDepartment =
    firstLower.includes("khoa") || firstLower.includes("phòng");
  const isSecondDepartment =
    secondLower.includes("khoa") || secondLower.includes("phòng");
...
function resolveDeviceImageUrl(
...
function buildDeviceImageCandidates(
  rawUrl: string | null | undefined,
  apiBaseUrl: string,
) {
...
export function DevicesPage({ onDataChanged }: DevicesPageProps) {
  ...
  const sortedDevices = useMemo(() => {
    return [...devices].sort((first, second) => {
      const nameCompare = String(first.name || "").localeCompare(
        String(second.name || ""),
        "vi",
        {
          sensitivity: "base",
  ...
  useEffect(() => {
    ...
    const isExistingStatus = statusOptions.some(
  ...
  const loadDevices = async (options?: { silent?: boolean }) => {
    try {
      ...
      const response = await fetch(
        `${apiBaseUrl}/api/devices?${params.toString()}`,
        {
          cache: "no-store",
        },
      );
      ...
      const normalizedDevices = (data.devices || []).map((device) => ({
        ...
        imageUrl: resolveDeviceImageUrl(
          device.imageUrl ??
            (device as { image_url?: string | null }).image_url ??
            null,
          apiBaseUrl,
        ),
      }));
  ...
  useEffect(() => {
    try {
      ...
      const normalizedDepartmentName =
        normalizeDepartmentValue(parsedUser.departmentName) ||
        normalizeDepartmentValue(parsedUser.department);
  ...
  const creatorDepartment =
    String(loggedInUser.departmentName || "").trim() || null;
  const employeeDepartment = useMemo(
    () =>
      normalizeDepartmentValue(loggedInUser.departmentName) ||
      normalizeDepartmentValue(loggedInUser.department),
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      "devices-data-changed",
      handleDevicesChanged as EventListener,
    );
  ...
  const loadDeviceLogs = async (device: DeviceItem) => {
    ...
    try {
      ...
      const response = await fetch(
        `${apiBaseUrl}/api/transfers/device/${deviceId}/logs`,
        {
          cache: "no-store",
        },
      );
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${apiBaseUrl}/api/devices/${selectedDevice.id}`,
        {
          method: "PUT",
          headers: {
          ...
          body: JSON.stringify({
            ...
            liquidationValue:
              editForm.status === "inactive" || editForm.status === "thanh_ly"
                ? editForm.liquidationValue?.trim() || null
                : null,
            liquidationFacility:
              editForm.status === "inactive" || editForm.status === "thanh_ly"
                ? editForm.liquidationFacility?.trim() || null
                : null,
          }),
        },
      );
  ...
  const handleDeleteDevice = async (device: DeviceItem) => {
    ...
    try {
      ...
      const fullNameParam = encodeURIComponent(
        String(loggedInUser.fullName || loggedInUser.username || ""),
      );
      const response = await fetch(
  ...
  const normalizeCsvDate = (value: string) => {
    ...
    const isValidYmd = (
      ...
      if (
        !Number.isInteger(checkYear) ||
        !Number.isInteger(checkMonth) ||
        !Number.isInteger(checkDay)
      ) {
        return false;
      }
    ...
    const ddMMyyyyValid = isValidYmd(
      year,
      ddMMyyyyCandidate.month,
      ddMMyyyyCandidate.day,
    );
    const mmDDyyyyValid = isValidYmd(
      year,
      mmDDyyyyCandidate.month,
      mmDDyyyyCandidate.day,
    );
  ...
  const handleImportCsvDevices = async () => {
    ...
    const indexMap = {
      ...
      purchaseDate: findIndex([
        "ngaymua",
        "purchasedate",
        "purchasedate",
        "purchase_date",
      ]),
      warrantyExpiry: findIndex([
        "hethanbaohanh",
        "warrantyexpiry",
        "warranty_expiry",
      ]),
    ...
    try {
      ...
      const tasks = dataLines.map((line, lineIndex) => async () => {
        ...
        const warrantyExpiry = normalizeCsvDate(
          readCell(indexMap.warrantyExpiry),
        );
        const maintenanceInterval = readCell(indexMap.maintenanceInterval);
        const imageUrl = resolveDeviceImageUrl(
          readCell(indexMap.imageUrl) || null,
          apiBaseUrl,
        );
      ...
      for (
        ...
        results.forEach((result) => {
          if (!result.ok) {
            failedCount += 1;
            failedDetails.push(
              `Dòng ${result.lineNumber}: ${result.name} - ${result.message}`,
            );
            return;
          }
  ...
  const handleCreateDevice = async () => {
    const requiredChecks = [
      { key: "name", label: "Tên thiết bị", value: createForm.name.trim() },
      {
        key: "manufacturer",
        label: "Hãng sản xuất",
        value: createForm.manufacturer.trim(),
      },
      ...
      { key: "quantity", label: "Số lượng", value: createForm.quantity.trim() },
      {
        key: "purchaseDate",
        label: "Ngày mua",
        value: createForm.purchaseDate.trim(),
      },
      {
        key: "warrantyExpiry",
        label: "Hết hạn bảo hành",
        value: createForm.warrantyExpiry.trim(),
      },
      {
        key: "maintenanceInterval",
        label: "Chu kỳ bảo trì",
        value: createForm.maintenanceInterval.trim(),
      },
    ];
    ...
    <div className="space-y-6">
      ...
      >
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            >
              {isDeletingId === selectedDeleteDevice?.id
                ? "Đang xóa..."
                : "Xóa"}
            </AlertDialogAction>
          </AlertDialogFooter>
            ...
            <table className="min-w-[1500px] table-auto border-collapse text-center align-middle [&_thead]:sticky [&_thead]:top-0 [&_thead]:z-20 [&_thead]:bg-[#a7d8c8] [&_thead_th]:bg-[#a7d8c8] [&_thead_th]:border-b [&_thead_th]:border-[#7fc8af] [&_thead_th]:font-bold [&_thead_th]:text-[#0f3b2f] [&_thead_th:first-child]:rounded-tl-lg [&_thead_th:last-child]:rounded-tr-lg [&_tbody]:bg-card [&_tbody_td]:border-b [&_tbody_td]:border-border/60 [&_th]:border-r [&_th]:border-border/60 [&_td]:border-r [&_td]:border-border/60 [&_th:last-child]:border-r-0 [&_td:last-child]:border-r-0">
              <thead>
                <tr className="border-b border-primary/45 text-center">
                  <th className="w-16 px-4 py-4 text-center text-sm font-bold text-foreground">
                    STT
                  </th>
                  <th className="w-24 px-4 py-4 text-sm font-bold text-foreground text-center whitespace-nowrap">
                    Mã Serial
                  </th>
                ...
                {visibleDevices.map((device, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-4 text-center text-sm font-semibold text-foreground whitespace-nowrap">
                        {device.code}
                      </td>
                      ...
                      <td className="px-4 py-4 text-center">
                        <div className="flex flex-col items-center gap-2">
                          <Badge
                            className={`${statusClasses[device.status] || "bg-muted text-muted-foreground"} text-sm w-fit`}
                          >
                            {statusLabels[device.status] || device.status}
                          </Badge>
                          {warrantyAlert && (
                            <Badge
                              className={`${warrantyAlert.className} text-xs w-fit`}
                            >
                              {warrantyAlert.label}
                            </Badge>
                          )}
                          ...
                          >
                            ...
                            {hasPermission(
                              ...
                              <DropdownMenuItem
                                ...
                                onClick={() =>
                                  handleRequestDeleteDevice(device)
                                }
                              >
      ...
      <Dialog open={isDetailDialogOpen} onOpenChange={setIsDetailDialogOpen}>
        <DialogContent className="max-h-[92vh] w-[92vw] overflow-hidden bg-card border-border sm:max-w-[860px]">
          ...
          {selectedDevice &&
            (() => {
              const imageCandidates = buildDeviceImageCandidates(
              ...
              return (
                <Tabs
                  value={detailTab}
                  onValueChange={(value) =>
                    setDetailTab(value as "info" | "logs")
                  }
                >
                      ...
                      <div className="rounded-lg border border-border p-3">
                        ...
                        <div className="mt-2 grid grid-cols-1 gap-x-5 gap-y-1.5 text-[13px] md:grid-cols-2">
                          ...
                          <div className="flex items-start gap-2">
                            ...
                            <span className="break-words">
                              {
                                buildDepartmentDisplay(selectedDevice)
                                  .department
                              }
                            </span>
                          </div>
                          ...
                          <div className="flex items-start gap-2">
                            ...
                            <span className="break-words">
                              {formatDeviceValue(selectedDevice.value)}
                            </span>
                          </div>
                          <div className="flex items-start gap-2">
                            <span className="w-24 shrink-0 font-semibold">
                          ...
                          <div className="flex items-start gap-2">
                            ...
                            <span className="break-words">
                              {formatDate(selectedDevice.warrantyExpiry)}
                            </span>
                          </div>
                          <div className="flex items-start gap-2">
                            <span className="w-24 shrink-0 font-semibold">
                            ...
                            <span className="break-words">
                              {formatMaintenanceInterval(
                                selectedDevice.maintenanceInterval,
                              )}
                            </span>
                          </div>
                          <div className="flex items-start gap-2">
                            ...
                            <span className="break-words">
                              {String(selectedDevice.createdBy || "-")}
                            </span>
                          </div>
                        </div>
                              ...
                              <img
                                ...
                                onError={(event) => {
                                  const currentIndex = Number(
                                    event.currentTarget.dataset.fallbackIndex ||
                                      "0",
                                  );
                  ...
                  <TabsContent
                    value="logs"
                    className="max-h-[62vh] overflow-y-auto pt-2 pr-1"
                  >
                    {isLogsLoading && (
                      <p className="text-sm text-muted-foreground">
                        Đang tải nhật kí thiết bị...
                      </p>
                    )}

                    {!isLogsLoading && deviceLogs.length === 0 && (
                      <p className="text-sm text-muted-foreground">
                        Chưa có nhật kí điều chuyển, bảo trì hoặc sửa chữa.
                      </p>
                    )}
                ...
                <div className="space-y-1">
                  ...
                  <Input
                    ...
                    onChange={(event) =>
                      setEditForm((prev) => ({
                        ...prev,
                        liquidationFacility: event.target.value,
                      }))
                    }
                  />
                </div>
                <div className="space-y-1">
                  ...
                  <Input
                    ...
                    onChange={(event) =>
                      setEditForm((prev) => ({
                        ...prev,
                        liquidationValue: event.target.value,
                      }))
                    }
                  />
                </div>
        ...
        <DialogContent className="max-h-[85vh] overflow-y-auto bg-card border-border">
          ...
          <Tabs value={createTab} onValueChange={setCreateTab}>
            ...
            <TabsContent value="manual" className="space-y-3 pt-3">
              <div className="space-y-1">
                <Label>
                  Số lượng thiết bị <span className="text-destructive">*</span>
                </Label>
                <Input
                  type="number"
                  min={1}
              ...
              <div className="space-y-1">
                <Label>
                  Hãng sản xuất <span className="text-destructive">*</span>
                </Label>
                <Input
              ...
              <div className="space-y-1">
                <Label>Vị trí</Label>
                <Input
                  value="Tự động cập nhật khi cấp phát/điều chuyển"
                  disabled
                  readOnly
                />
              </div>
              ...
              <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                ...
                <div className="space-y-1">
                  <Label>
                    Hết hạn bảo hành <span className="text-destructive">*</span>
                  </Label>
                  <Input
                    type="date"
                    value={createForm.warrantyExpiry ?? ""}
                    onChange={(event) =>
                      setCreateForm((prev) => ({
                        ...prev,
                        warrantyExpiry: event.target.value,
                      }))
                    }
                  />
                </div>
            ...
            <TabsContent value="csv" className="space-y-3 pt-3">
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                <Button
                  onClick={handleImportCsvDevices}
                  disabled={isImportingCsv}
                >
                  ...
                  return (
                    ...
                    >
                      <Checkbox
                        ...
                        onCheckedChange={() =>
                          toggleExportDepartment(department)
                        }
                      />
